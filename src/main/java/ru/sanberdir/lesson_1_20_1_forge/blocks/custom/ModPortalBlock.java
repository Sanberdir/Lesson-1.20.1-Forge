...
import ru.sanberdir.lesson_1_20_1_forge.world.portal.ModTeleporter;
    ...
    private void handleKaupenPortal(Entity player, BlockPos pPos) {
        if (player.level() instanceof ServerLevel serverlevel) {
            ...
            ResourceKey<Level> resourcekey = player.level().dimension() == ModDimensions.CUSTOMDIM_LEVEL_KEY ?
                    Level.OVERWORLD : ModDimensions.CUSTOMDIM_LEVEL_KEY;

            ServerLevel portalDimension = minecraftserver.getLevel(resourcekey);
            if (portalDimension != null && !player.isPassenger()) {
                if(resourcekey == ModDimensions.CUSTOMDIM_LEVEL_KEY) {
